...
pdf_load_simple_font(fz_context *ctx, pdf_document *doc, pdf_obj *dict, pdf_obj *fftype)
{
	...
	fz_try(ctx)
	{
		...
		/* FITZ PORT FALLBACK: Synthesize ToUnicode from BaseEncoding + Adobe Glyph List (AGL) */
		if (!fontdesc->to_unicode && estrings)
		{
			int ucs;
			fontdesc->to_unicode = pdf_new_identity_cmap(ctx, 0, 1);
			for (int c = 0; c < 256; c++) {
				if (estrings[c]) {
					ucs = fz_unicode_from_glyph_name(estrings[c]);
					if (ucs > 0) {
						int gid = fontdesc->cid_to_gid ? fontdesc->cid_to_gid[c] : c;
						pdf_map_range_to_range(ctx, fontdesc->to_unicode, gid, gid, ucs);
					}
				}
			}
			pdf_sort_cmap(ctx, fontdesc->to_unicode);
		}
